...
val version = sys.env.get("GITHUB_REF").collect { case s"refs/tags/$tag" => tag }.getOrElse("dev")
...
final class Main(config: Config)(using log: Logger[IO]):
  ...
  /** Run the main loop (log errors, never exit)
    */
  def loop(server: ServerConfig): IO[Nothing] =
    run.handleErrorWithLog *>
      log.info(show"Sleeping for ${server.interval}") *>
      IO.sleep(server.interval) >>
      loop(server)
